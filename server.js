...
const binaryPath = path.join(
  ...
  "node_modules/sqlite3/lib/binding/node-v108-win32-x64/node_sqlite3.node"
);
...
// Gets the local ip address of this machine in this network
...
async function startServer() {
  ...
  db.serialize(() => {
    ...
    db.run(
      "CREATE TABLE IF NOT EXISTS templogs (id INTEGER PRIMARY KEY AUTOINCREMENT, name, temperature REAL, created_at TEXT)"
    );
  });
  ...
  app.get("/templogs", (req, res) => {
    db.all(
      "SELECT * FROM templogs ORDER BY created_at DESC LIMIT ?",
      [15],
      (err, rows) => {
        if (err) {
